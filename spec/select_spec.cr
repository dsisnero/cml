...
module CML
  describe "select macro" do
    it "selects the first ready event and runs the correct handler" do
      ...
      result = CML.select([
        {ch1.recv_evt, ->(x : Int32?) { x.not_nil! + 1 }},
        {ch2.recv_evt, ->(y : Int32?) { y.not_nil! * 2 }},
      ])
      result.should eq(84)
    end
    ...
    it "selects the first to complete if none are ready" do
      ...
      result = CML.select([
        {ch.recv_evt, ->(x : Int32?) { x.not_nil! * 3 }},
        {CML.never(Int32), ->(y : Int32?) { y || 0 }},
      ])
      result.should eq(21)
    end
    ...
    it "works with immediate events" do
      result = CML.select([
        {CML.always(5), ->(x : Int32?) { x.not_nil! * 2 }},
        {CML.always(10), ->(y : Int32?) { y.not_nil! + 1 }},
      ])
