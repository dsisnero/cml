...
Crystal 1.19 introduces several important concurrency-related features that
could benefit the CML implementation. This document outlines a plan to integrate
these features while maintaining compatibility and performance.
...
*   New synchronization primitives: `Sync::Mutex`, `Sync::RWLock`,
  `Sync::ConditionVariable`, `Sync::Exclusive`, `Sync::Shared`
* **Impact**: Potential need for thread-safe synchronization when CML runs with
  multiple OS threads
...
Create a wrapper module that selects appropriate mutex based on compilation
flags:
...
1.  Crystal 1.19.0 Release Notes:
