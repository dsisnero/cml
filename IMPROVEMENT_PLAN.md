...
* **Root Cause**: `__choose_from_array` method calls `choose(events)` which
  expands to call `__choose_from_array` again
...
* **Macro Expansion**: `CML.choose([evt1, evt2])` →
  `CML.__choose_from_array([evt1, evt2])`
* **Method Implementation**: `__choose_from_array` calls `choose(events)` →
  macro expansion → infinite recursion
* **Type System Issue**: Return type annotation `Event(T)` doesn't match actual
  return type
